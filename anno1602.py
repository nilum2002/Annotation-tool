...
basedir = "/home/" + getuser() + "/laser-dumps/"
# Where to save the detections to.
# TODO: Describe better!
savedir = "/home/" + getuser() + "/laser-dumps/annotations/"
...
class MouseCircle(AxesWidget):
    ...
    def onmove(self, e):
        if self.ignore(e) or not self.canvas.widgetlock.available(self):
            return
        # Off the axes there is no data coordinate for the circle. Matplotlib
        # used to render a None centre as invisible, but since 3.x it raises
        # inside the patch transform, so hide the circle explicitly instead.
        if e.xdata is None or e.ydata is None:
